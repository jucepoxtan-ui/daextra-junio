...
<html>
	...
	<?php 
	$sql= "SELECT * from noticias where id=".$_GET["noticia"]."";	
	...
	while ($resultados = mysql_fetch_assoc($query)){
		...
		$tags_actuales = $resultados["tags"];				$video = $resultados["video"];
	}mysql_free_result($query);
	
	$query = mysql_query("SELECT nombre FROM imagenes_noticias where noticia=".$_GET["noticia"]." and principal=1");
	$results = mysql_fetch_array($query); mysql_free_result($query);
	$superior = $results['nombre'];
	
	$query = mysql_query("SELECT nombre FROM imagenes_noticias where noticia=".$_GET["noticia"]." and principal=2");
	$results = mysql_fetch_array($query); mysql_free_result($query);
	$media = $results['nombre'];
	
	$query = mysql_query("SELECT nombre FROM imagenes_noticias where noticia=".$_GET["noticia"]." and principal=3");
	$results = mysql_fetch_array($query); mysql_free_result($query);
	$inferior = $results['nombre'];
	?>
						<input type="hidden" value="<?php echo $_GET["noticia"]; ?>" id="item">
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="panel panel-default">
                                    <div class="panel-heading"><h3 class="panel-title">Editar noticia</h3></div>
                                    <div class="panel-body"> 
										<form role="form">
											<div class="form-group">
												<label for="titulo">T&iacute;tulo</label>
												<input type="text" class="form-control" value="<?php echo $titulo; ?>" id="titulo" placeholder="T&iacute;tulo">
											</div>		
											...
											<div class="col-sm-12">
												<div class="form-group">
													...
													<textarea class="form-control" id="intro"><?php echo $texto_corto; ?></textarea>
												</div>											
											</div>											
											<div class="col-sm-12">
												<div class="form-group">
													<label for="texto">Texto completo</label>
													<textarea class="summernote" id="texto"><?php echo $texto_completo; ?></textarea>
												</div>											
											</div>
											<div class="col-sm-4">	
												<div class="form-group">
													...
													<img src="imagenes/<?php echo $superior; ?>" style="width:100%;">
												</div>
											</div>	
											<div class="col-sm-4">
												<div class="form-group">
													...
													<?php if($media != NULL){ ?>
													<img src="imagenes/<?php echo $media; ?>" style="width:100%;">
													<?php } ?>
												</div>
											</div>												
											<div class="col-sm-4" style="display: none;">
												<div class="form-group">
													...
													<img src="imagenes/<?php echo $inferior; ?>" style="width:100%;">
												</div>
											</div>
										</form>
                                    </div>
								...
								<div class="panel panel-default">
									...
									<div class="panel-body todoapp">
										<ul class="list-group no-margn nicescroll todo-list" style="max-height: 288px; overflow: hidden; outline: none;" id="todo-list" tabindex="5002">
											<?php
												function tags($tags_actuales){
													$array_tags_actuales = explode(",",$tags_actuales);
											...
											<li class="list-group-item li-<?php echo $id[$i]; ?>">
												<div class="checkbox checkbox-primary">
													<input class="todo-done check-tags" id="tag-<?php echo $id[$i]; ?>" value="<?php echo $id[$i]; ?>" type="checkbox" <?php if(in_array($id[$i],$array_tags_actuales)){ echo "checked"; }?>>
						...
						<div class="row">
                            <div class="col-sm-12">
                                <div class="panel panel-default">
                                    <div class="panel-heading"><h3 class="panel-title">Comentarios</h3></div>
                                    <div class="panel-body"> 
										<?php
										// Funcion para los pagos de los niños
										function comentarios(){
											$sql= "select * from comentarios where noticia='".$_GET["noticia"]."'";
											$query=mysql_query($sql) or die(mysql_error());			
												while ($resultados = mysql_fetch_assoc($query)){
												$id[] = $resultados["id"];
												...
												<div class="col-lg-4 comentario-<?php echo $id[$i]; ?>">
												    <i class="fa fa-times" style="position: absolute;right: 5%;top: 7%;font-size: 22px;" onclick="EliminaComentario(<?php echo $id[$i]; ?>);"></i>
													<div class="panel panel-color panel-info">
														<div class="panel-heading">
															<h3 class="panel-title"><?php echo $nombre[$i]; ?> - <?php echo $email[$i]; ?></h3>
														</div>
														<div class="panel-body">
        ...
        <script>			$('#video').change(function(){				var url = $(this).val();				$('iframe').fadeIn('slow');				if (url.indexOf('youtube.com') > -1) {					var url = url.replace("watch?v=", "embed/");					$('iframe').attr('src',url);				}else if (url.indexOf('vimeo.com') > -1) {					var url = url.replace("vimeo.com", "player.vimeo.com/video");					$('iframe').attr('src',url);				}else{					alert('La URL del video es invalida, favor de corregirla o en su caso eliminarla.');				}			});
            ...
            function EliminaComentario(id){
                ...
                var r = confirm("Esta seguro de eliminar el comentario?");
                if (r == true) {
                    $.ajax({
        				url: "ajax_noticias.php",
        				type: 'POST',
        				data:{id:id,eliminacomentario:1}, //Le pasamos el objeto que creamos con los archivos
        				cache: false,
        				success: function(html){
        					$('.comentario-'+identificador).fadeOut('slow');
        				}
        			})
                }
            }
		...
		$('#guardar').click(function() {
			...
			// Variables
				...
				datos.append('intro', intro);
				datos.append('id', id);								datos.append('video', video);
				datos.append('edicion', 1);
